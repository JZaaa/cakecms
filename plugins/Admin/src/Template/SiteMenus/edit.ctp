...
<form action="<?php echo $this->Url->build(['plugin' => 'Admin', 'controller' => 'SiteMenus', 'action' => 'edit', $id])?>" method="post" data-toggle="ajaxform" autocomplete="off">
    ...
    <div class="form-group">
        ...
        <input id="subname" name="subname" value="<?php echo $data->subname?>" type="text" class="form-control">
    </div>
    <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select id="list_tpl" name="list_tpl" class="chosen-select form-control" data-toggle="selectpicker">
                    ...
                    <?php foreach ($listTpls as $key => $item):?>
                        <option value="<?php echo $key?>" <?php if ($data->list_tpl == $key) echo 'selected'?>><?php echo $item?></option>
                    <?php endforeach;?>
                </select>
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <select id="content_tpl" name="content_tpl" class="chosen-select form-control" data-toggle="selectpicker">
                    <?php foreach ($contentTpls as $key => $item):?>
                        <option value="<?php echo $key?>" <?php if ($data->content_tpl == $key) echo 'selected'?>><?php echo $item?></option>
                    <?php endforeach;?>
                </select>
            </div>
        </div>
    ...
    <div class="form-group">
        ...
        <select id="status" name="status" class="chosen-select form-control" data-toggle="selectpicker">
            <?php foreach ($status as $key => $item):?>
                <option value="<?php echo $key?>" <?php if ($data->status == $key) echo 'selected'?>><?php echo $item?></option>
            <?php endforeach;?>
        </select>
    </div>
    <div class="uploader form-group" data-toggle="upload" data-url="<?php echo $this->Url->build(['plugin' => 'Admin', 'controller' => 'Upload', 'action' => 'image'])?>" data-response-handler="responseHandler" data-file="<?php if (!empty($data->pic)) echo $this->Url->webroot($data->pic)?>" data-delete-action-on-done="deleteActionOnDone">
        <label>图片</label>
    ...
    <div class="form-group">
        ...
        <input id="link" name="link" value="<?php echo $data->link?>" placeholder="http://" type="text" class="form-control">
    </div>
    <div class="form-group">
